...
public final class ClientSession: @unchecked Sendable {
    ...
    /// Desk UI (layout-ux R37/R38): ENTER landed — the edge is this machine's
    /// return edge and the focus is now here. On the transport queue.
    public var onEnter: (@Sendable (EdgeSide) -> Void)?
    /// Desk UI (layout-ux R38): LEAVE landed — focus went back to the server.
    /// On the transport queue.
    public var onLeave: (@Sendable () -> Void)?
    ...
    public init(
        ...
        sentinelEffects: SentinelEffects? = nil,
        ...
        self.sentinel = CursorSentinel(effects: sentinelEffects ?? .live(
    ...
    /// Split from `wire()` so tests can feed synthetic transport events
    /// (`apply` no-ops without Accessibility — safe off-device).
    func handle(_ event: TransportEvent) {
        ...
        case .connected:
            ...
            sentinel.engage(park: cursorLocation())
            onState?(.connected(peer: "servidor"))
        case .disconnected:
        ...
        case let .messages(messages):
            ...
            metrics.bump(.datagramsReceived)
            for message in messages {
                switch message {
                case .clipFiles, .filePull:
                    onFileMessage?(message)
                default:
                    if case let .enter(_, _, edge) = message {
                        onEnter?(edge)
                    } else if case .leave = message {
                        onLeave?()
                    }
                    metrics.bump(.injectedMessages)
