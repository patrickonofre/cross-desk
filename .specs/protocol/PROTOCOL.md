...
- **Duas fontes de código, mesmo HKDF** (ciclo de vida do pareamento):
  1. **Token curto de pareamento** — gerado e exibido pelo servidor não-pareado (6 chars, alfabeto sem ambíguos `23456789ABCDEFGHJKMNPQRSTVWXYZ`, exibido `XXX-XXX`, ≈29 bits); usuário digita no cliente uma vez. Vale só para a janela de pareamento.
...
- v0.1 (2026-07-06): token curto de pareamento reduzido de 8 para 6 chars (§1: `XXXX-XXXX` → `XXX-XXX`, ≈39 → ≈29 bits) — decisão do usuário, troca de UX (token mais curto de digitar) por menos entropia; token nunca trafega no fio (só deriva PSK localmente nos dois lados) e é substituído pelo segredo rotacionado de 128 bits após o 1º handshake (R29), então o risco residual é só a janela de pareamento (já documentado no §1). Não muda o formato do fio nem os golden vectors (PAIR_SET/PAIR_ACK carregam o segredo de 32 hex chars, não o token). `proto_version` inalterado (=1).
